...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored,response,false)){
      message.info("Successfully deleted address")
      this.setState({addresses:new_add})
    }
    else{
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
      if (response) {
        ...
        localStorage.setItem('balance',balance-this.cartRef.current.calculateTotal())
