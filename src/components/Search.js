...
class Search extends React.Component {
   ...
   */
  search = (text) => {
    var filter= this.products.filter(item=> item.name.toUpperCase().includes(text.toUpperCase()) || item.category.toUpperCase().includes(text.toUpperCase()))
    this.setState({filteredProducts:filter})
    //console.log("Filetered",this.state.filteredProducts)
  };
  ...
  getProducts = async () => {
    let productlist=[]
    try{
      productlist = await  this.performAPICall()
    }
    catch{
      productlist=[]
    }
    if(productlist && productlist.length>0){
      //message.info("Products"+productlist.length)
      this.products=productlist
      this.setState({filteredProducts:productlist})
        ...
        <Product
          ...
          addToCart={() => {
            if (this.state.loggedIn) {
              ...
              this.cart.current.pushToCart(product._id,1,true) 
            }
            else{
  ...
  render() {
    ...
    return (
      <>
        ...
        <Header history={this.props.history} Search={this.search} debounceSearch={this.debounceSearch}>
          ...
          {this.state.loggedIn && this.products.length && (
            ...
            >
              <div>
                ...
                {this.state.loggedIn  &&( 
                      <Cart  ref={this.cart} checkout={this.checkout} products={this.products} history={this.props.history} token={localStorage.getItem('token')} loggedIn={this.state.loggedIn}>
